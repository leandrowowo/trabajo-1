...
void printNumber(int *num, int size) {
    ...
    for (int i = 0; i < size; i++) {
        if (num[i] != 0) leadingZero = 0;
        if (!leadingZero) printf("%d", num[i]);
    }
    if (leadingZero) printf("0");
}


int divideBy2(int *num, int size)
{
    int current, carry;
    
    carry = 0; // llevada del dígito anterior
    
    for (int i = 0; i < size; i++)
    {
        current = carry * 10 + num[i];
        num[i] = current / 2;
        carry = current % 2;
    }

    return carry; // residuo de la división
}

int adjustSize(int *num, int size) {
    int newSize = size;
    while (newSize > 1 && num[0] == 0) {
...
{
    int *a, *b, resto, sizeA, sizeB, sizeMax;
    int *partial_sum;
    int i, j;
    ...
    partial_sum = (int *)calloc(sizeMax, sizeof(int));

    while(!(sizeA == 1 && a[0] == 0)) 
    {
        printNumber(a, sizeA);
        resto = divideBy2(a, sizeA);
        printf("\tResto: %d\n", resto);
        sizeA = adjustSize(a, sizeA);

        if(resto == 1)
        {
            for(i = sizeA; i >= 0; i = i - 1)
            {
                partial_sum[i] = partial_sum[i] + a[i];
            }
        }

        printf("\n");
        printNumber(partial_sum, sizeMax);
        printf("\n");
    }
