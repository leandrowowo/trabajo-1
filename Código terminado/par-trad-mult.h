...
void *Process_tradMult(void *p)
{
    ...
    printf("From %d - Times: CPU time %f Wall time %ld\n", me->thread_id, CPU_time, wall_time);
...
void Traditional(int k, int mode)
{
    ...
    struct Message **mess;
    int numthreads, i, j, result_size, chunk_size, rem, index;
    ...
    for(i = 0; i < numthreads; i = i + 1)
    {
        ...
        pthread_create(&thread[i], &attribute, Process_tradMult, (void *)mess[i]);
    }
